...
<dependency>
  ...
  <scope>system</scope>
  <systemPath>${project.basedir}/libs/crypto-core-0.1.0.RC1.jar</systemPath>
</dependency>
```
...
// Encrypt Hash (암호화된 Hash Key와 알고리즘으로 문자열을 Hash 처리)
// Hash Key가 없으면 Hash Key 없이 Hash 처리
// 입력된 알고리즘이 없으면 SHA512 알고리즘이 기본으로 적용됨
String encryptHash = session.encrypt_id(plaintext, 400);
// Hash (Hash 알고리즘(SHA256)만을 이용해 Hash 처리)
String hash = session.hash(plaintext);
// Hash Algorithm (입력된 Hash 알고리즘(SHA256, SHA384, SHA512, SHA512_256)을 이용해 Hash 처리)
String hashAlg = session.hash(plaintext, "SHA512");
// Hash Algorithm Key (Hash Key와 입력된 Hash 알고리즘(SHA256, SHA384, SHA512, SHA512_256)을 이용해 Hash 처리)
...
class CryptoFactoryConfigTest {
    ...
    void base_Initializing_cryptoSession() {
        ...
        String encryptHash = cryptoSession.encrypt_id(plaintext, 400);
        String hash = cryptoSession.hash(plaintext);
        String hashAlg = cryptoSession.hash(plaintext, "SHA512");
        String hashAlgKey = cryptoSession.hash(plaintext, EncryptUtils.generateKey(16).getBytes(StandardCharsets.UTF_8), "SHA512_256");
        
        String encryptHotel = secondCryptoSession.encrypt(plaintext);
        String decryptHotel = secondCryptoSession.decrypt(encryptHotel);
        String encryptHashHotel = secondCryptoSession.encrypt_id(plaintext, 400);
        String hashHotel = secondCryptoSession.hash(plaintext);
        String hashAlgHotel = secondCryptoSession.hash(plaintext, "SHA512");
        String hashAlgKeyHotel = secondCryptoSession.hash(plaintext, EncryptUtils.generateKey(16).getBytes(StandardCharsets.UTF_8), "SHA512_256");
    }
}
```
...
`$TOMCAT_HOME/lib` 폴더에 `crypto-core-0.1.0.RC1.jar` 파일을 직접 추가
...
```groovy
dependencies {
    compileOnly 'com.freelife.crypto:crypto-core:0.1.0.RC1'
    testCompileOnly 'com.freelife.crypto:crypto-core:0.1.0.RC1'
}
```
...
- [번외 - `CryptoSession` Factory 설정 및 사용 방법](docs/ETC.md)