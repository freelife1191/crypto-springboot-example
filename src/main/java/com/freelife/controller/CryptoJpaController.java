...
@RestController
@RequestMapping("/jpa")
@Tag(name = "Crypto Example JPA API", description = "JPA(Converter) 암복호화 예제 API")
public class CryptoJpaController {

    private final CryptoService cryptoService;

    public CryptoJpaController(@Qualifier("cryptoJpaService") CryptoService cryptoService) {
        this.cryptoService = cryptoService;
    }

    @Operation(summary = "JPA 회원 추가", description = "## JPA 회원 추가 설명\n" +
    ...
    @Operation(summary = "JPA 회원 조회", description = "# JPA 회원 조회 설명\n" +
    ...
    @Operation(summary = "JPA 회원 수정", description = "# JPA 회원 수정 설명\n" +
            "- `password` 는 `Converter` 를 통해 암호화되서 저장되고 조회시 복호화되어 반환됨\n" +
            "- `encPassword` 를 통해 암호화된 `password` 를 확인할 수 있음"
    )
