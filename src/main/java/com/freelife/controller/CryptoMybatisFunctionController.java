...
@Slf4j
@RestController
@RequestMapping("/mybatis/function")
@Tag(name = "Crypto Example Mybatis Function API", description = "Mybatis Function 암복호화 예제 API")
@RequiredArgsConstructor
public class CryptoMybatisFunctionController {

    private final CryptoFunctionService cryptoService;
    ...
    @Operation(summary = "Mybatis Bulk 회원 추가 (Petra)", description = "# Mybatis Bulk 회원 추가 설명\n" +
            "- `password` 는 MySQL 에 등록된 Function 을 통해 암호화되서 저장되고 조회시 복호화되어 반환됨\n" +
            "- `encPassword` 를 통해 암호화된 `password` 를 확인할 수 있음"
    )
    @PostMapping("/members-petra")
    public ResponseEntity<Integer> saveAllPetra(
            @Parameter(description = "회원 수", required = true, example = "100")
            @RequestParam(value = "count", defaultValue = "100") Integer count
    ) {
        return ResponseEntity.ok(cryptoService.saveAllPetra(count));
    }
    ...
    @Operation(summary = "Mybatis 회원 조회 (Petra)", description = "# Mybatis 회원 조회 설명\n" +
            "- `password` MySQL 에 등록된 Function 을 통해 조회시 복호화되어 반환됨\n" +
            "- `encPassword` 를 통해 암호화된 `password` 를 확인할 수 있음"
    )
    @GetMapping("/member-petra")
    public ResponseEntity<List<ResponseMemberDto>> selectMemberPetra(
            @Parameter(description = "회원 수", required = true, example = "100")
            @RequestParam(value = "count", defaultValue = "100") Integer count
    ) {
        return ResponseEntity.ok(cryptoService.findMembersPetra(count));
    }
